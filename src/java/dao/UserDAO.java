 ...
 * @author Admin
 */
public class UserDAO implements ICommon<User>{
    Session ss;
    ...
    public DungChung.ReturnMessage getById(String id) {
        ...
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
            User data = (User) ss.get(User.class, id);
            ss.close();
            msg.data = data;
        } catch (Exception e) {
            msg.setError("Error " + e.toString());
    ...
    @Override
    public DungChung.ReturnMessage setData(User entity) {
        DungChung.ReturnMessage msg = new DungChung.ReturnMessage(DungChung.ReturnMessage.eState.SUCCESS);
        msg.setStatus();
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
    ...
    public DungChung.ReturnMessage removeData(String id) {
        ...
        try {
            ss = HibernateUtil.getSessionFactory().openSession();
            User data = (User) this.getById(id).data;
            ss.beginTransaction();
            ss.delete(data);
            ss.getTransaction().commit();
            ss.close();
